...
type Rol = "ADMIN" | "OPERADOR" | string;
...
export default function UsuariosPage() {
  ...
  // Carga de datos — se re-ejecuta cada vez que refreshKey cambia
  useEffect(() => {
    let cancelled = false;
  ...
  // Escucha el evento de refresh disparado desde alta/modificacion
  useEffect(() => {
  ...
  const cargarUsuarios = () => setRefreshKey((k) => k + 1);
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .btn-alta:active { transform: translateY(0); }
        ...
        .user-handle { font-family: 'IBM Plex Mono', monospace; font-size: 0.7rem; color: #4ade80; margin-top: 1px; }

        .badge { display: inline-flex; align-items: center; gap: 4px; font-family: 'IBM Plex Mono', monospace; font-size: 0.65rem; font-weight: 500; letter-spacing: 0.05em; padding: 3px 9px; border-radius: 4px; white-space: nowrap; }
        .badge-super { background: #1a1040; color: #a78bfa; border: 1px solid #a78bfa30; }
        .badge-admin { background: #0f2820; color: #4ade80; border: 1px solid #4ade8030; }
        .badge-operador { background: #0f1e2d; color: #60a5fa; border: 1px solid #60a5fa30; }

        .meta-text { font-size: 0.78rem; color: #64748b; }
        .meta-date { font-family: 'IBM Plex Mono', monospace; font-size: 0.68rem; color: #334155; margin-top: 2px; }
        ...
        .btn-action.danger:hover { background: #2d1215; color: #f87171; border-color: #7f1d1d40; }
        ...
        /* TOAST */
        .toast { position: fixed; bottom: 1.5rem; right: 1.5rem; background: #0f2820; border: 1px solid #4ade8050; color: #4ade80; font-size: 0.8rem; padding: 0.65rem 1rem; border-radius: 8px; display: flex; align-items: center; gap: 0.5rem; font-family: 'IBM Plex Sans', sans-serif; animation: slideUp 0.3s ease; z-index: 100; }
      ...
      <div className="shell">
        ...
        <main className="main">
          ...
          <nav className="breadcrumb">
            ...
            <span className="breadcrumb-current">Usuarios administrativos</span>
          </nav>
          ...
          <div className="page-header">
            <div>
              ...
              <p className="page-subtitle">Registro y administración de usuarios con acceso al sistema</p>
            </div>
            <Link href="/usuarios/alta" className="btn-alta">
          ...
          <div className="toolbar">
            <div className="search-wrap">
              ...
              <input className="search-input" type="text" placeholder="Buscar por nombre o usuario..." value={busqueda} onChange={(e) => setBusqueda(e.target.value)} />
            </div>
            ...
            <span className="count-tag">
              {loading ? "…" : `${usuariosFiltrados.length} usuario${usuariosFiltrados.length !== 1 ? "s" : ""}`}
            </span>
          </div>
                ...
                <tbody>
                  {loading && [1,2,3].map(i => (
                    <tr key={i} className="skeleton-row">
                      ...
                      <td><div className="skeleton" style={{ height: "22px", width: "90px" }} /></td>
                      <td><div className="skeleton" style={{ height: "16px", width: "120px" }} /></td>
                      <td><div className="skeleton" style={{ height: "16px", width: "110px" }} /></td>
                      <td><div className="skeleton" style={{ height: "28px", width: "120px" }} /></td>
                    </tr>
                  ))}
                  ...
                  {!loading && !error && usuariosFiltrados.length === 0 && (
                    <tr><td colSpan={5}>
                      <div className="empty-state">
                        <div className="empty-icon">
                          <svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="#1e2330" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M15 19.128a9.38 9.38 0 0 0 2.625.372 9.337 9.337 0 0 0 4.121-.952 4.125 4.125 0 0 0-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 0 1 8.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0 1 11.964-3.07M12 6.375a3.375 3.375 0 1 1-6.75 0 3.375 3.375 0 0 1 6.75 0Zm8.25 2.25a2.625 2.625 0 1 1-5.25 0 2.625 2.625 0 0 1 5.25 0Z" /></svg>
                        </div>
                        <p className="empty-title">No hay usuarios registrados</p>
                        <p className="empty-sub">El sistema aún no tiene usuarios administrativos dados de alta.</p>
                        <Link href="/usuarios/alta" className="btn-alta-outline">
                          ...
                          Dar de alta el primer usuario
                        </Link>
                      </div>
                    </td></tr>
                  )}

                  {!loading && !error && usuariosFiltrados.map((u) => (
                    <tr key={u.id}>
                      <td>
                        <div className="user-cell">
                          <div className="avatar">{`${u.nombre[0]}${u.apellido[0]}`.toUpperCase()}</div>
                          <div>
                            <div className="user-name">{u.nombre} {u.apellido}</div>
                            <div className="user-handle">@{u.nombreUsuario}</div>
                          </div>
                        </div>
                      </td>
                      <td><span className={ROL_STYLES[u.rol] ?? "badge badge-admin"}>{ROL_LABELS[u.rol] ?? u.rol}</span></td>
                      <td><div className="meta-text">{u.creadoPorNombre ?? <span style={{ color: "#334155" }}>Usuario inicial</span>}</div></td>
                      <td><div className="meta-date">{formatFecha(u.fechaCreacion)}</div></td>
                      <td>
                        <div className="actions">
                          {/* Solo se muestran acciones para usuarios que no son el inicial */}
                          {u.id !== 1 && (
                            <>
                              <Link href={`/usuarios/${u.id}/modificacion`} className="btn-action">
