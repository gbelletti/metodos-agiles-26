...
export default function ModificacionUsuarioPage() {
  ...
  const [form, setForm] = useState<FormData>({ nombre: "", apellido: "", nombreUsuario: "", nuevaContrasena: "", confirmarContrasena: "", rol: "" });
  ...
  useEffect(() => {
    fetch(`${API_URL}/api/usuarios/${id}`)
      ...
      .then((data) => setForm({ nombre: data.nombre ?? "", apellido: data.apellido ?? "", nombreUsuario: data.nombreUsuario ?? "", nuevaContrasena: "", confirmarContrasena: "", rol: data.rol ?? "" }))
  ...
  async function handleSubmit() {
    ...
    setLoadingSave(true);
    try {
      const body: Record<string, string> = { nombre: form.nombre, apellido: form.apellido, nombreUsuario: form.nombreUsuario, rol: form.rol };
      if (cambiarContrasena && form.nuevaContrasena) body.contrasena = form.nuevaContrasena;
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .toggle-section { display: flex; align-items: center; gap: 0.75rem; padding: 0.75rem 1rem; background: #0d0f14; border: 1px solid #1e2330; border-radius: 6px; cursor: pointer; transition: border-color 0.15s; margin-bottom: 1.25rem; user-select: none; }
        .toggle-section:hover { border-color: #334155; }
        .toggle-switch { width: 36px; height: 20px; background: #1e2330; border-radius: 10px; position: relative; transition: background 0.2s; flex-shrink: 0; }
        ...
        .skeleton { background: linear-gradient(90deg, #1e2330 25%, #252b3b 50%, #1e2330 75%); background-size: 200% 100%; animation: shimmer 1.4s infinite; border-radius: 6px; }
          ...
          <div className="form-card">
            {fetchError && (
              <div style={{ padding: "3rem 2rem", textAlign: "center" }}>
                <p style={{ color: "#f87171", fontWeight: 500, marginBottom: "0.5rem" }}>No se pudo cargar el usuario</p>
                <p style={{ fontSize: "0.78rem", color: "#475569" }}>{fetchError}</p>
              </div>
            )}
            ...
            {loadingData && !fetchError && (
              <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem" }}>
                  {[1,2,3,4].map(i => <div key={i} className="skeleton" style={{ height: "38px" }} />)}
                </div>
                <div className="skeleton" style={{ height: "12px", width: "25%", marginTop: "0.5rem" }} />
              ...
              <>
                ...
                <div className="form-grid">
                  <div className="form-group">
                    ...
                    <input type="text" value={form.nombre} onChange={(e) => handleChange("nombre", e.target.value)} className={errors.nombre ? "err" : ""} />
                    {errors.nombre && <span className="field-error">⚠ {errors.nombre}</span>}
                  </div>
                  <div className="form-group">
                    <label>Apellido <span className="required">*</span></label>
                    <input type="text" value={form.apellido} onChange={(e) => handleChange("apellido", e.target.value)} className={errors.apellido ? "err" : ""} />
                ...
                <div className="form-grid">
                  <div className="form-group full">
                    ...
                    <input type="text" value={form.nombreUsuario} onChange={(e) => handleChange("nombreUsuario", e.target.value.toLowerCase().replace(/\s/g, ""))} className={errors.nombreUsuario ? "err" : ""} />
                    {errors.nombreUsuario ? <span className="field-error">⚠ {errors.nombreUsuario}</span> : <span style={{ fontSize: "0.7rem", color: "#334155" }}>Debe ser único en el sistema.</span>}
                  </div>
                  <div className="form-group full">
                ...
                <div className="toggle-section" onClick={() => { setCambiarContrasena(!cambiarContrasena); setErrors(e => ({ ...e, nuevaContrasena: undefined, confirmarContrasena: undefined })); }}>
                  ...
                  <span className="toggle-label"><strong>Cambiar contraseña</strong> — dejá desactivado para mantener la actual</span>
                </div>
                  ...
                  <div className="form-grid">
                    <div className="form-group">
                      ...
                      <div className="input-wrap">
                        <input type={mostrarContrasena ? "text" : "password"} placeholder="Mínimo 8 caracteres" value={form.nuevaContrasena} onChange={(e) => handleChange("nuevaContrasena", e.target.value)} className={errors.nuevaContrasena ? "err" : ""} style={{ paddingRight: "2.5rem" }} />
